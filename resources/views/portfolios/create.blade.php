                ...
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    <form method="POST" action="{{ route('portfolios.store') }}" enctype="multipart/form-data" class="space-y-6">
                        ...
                        <div>
                            ...
                            <textarea id="deskripsi" name="deskripsi" rows="3" class="block mt-1 w-full border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 focus:border-indigo-500 dark:focus:border-indigo-600 focus:ring-indigo-500 dark:focus:ring-indigo-600 rounded-md shadow-sm" required>{{ old('deskripsi') }}</textarea>
                            <x-input-error :messages="$errors->get('deskripsi')" class="mt-2" />
                        </div>

                        <div>
                            <x-input-label for="foto" :value="__('Foto')" />
                            <input id="foto" type="file" name="foto" class="block mt-1 w-full border border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 focus:border-indigo-500 dark:focus:border-indigo-600 focus:ring-indigo-500 dark:focus:ring-indigo-600 rounded-md shadow-sm" accept="image/*" required />
                            <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">Upload a portfolio image (JPEG, PNG, JPG, GIF, max 2MB)</p>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div>
                                ...
                                <x-text-input id="harga_minimal" class="block mt-1 w-full" type="number" name="harga_minimal" :value="old('harga_minimal')" min="0" step="1000" required />
                                <x-input-error :messages="$errors->get('harga_minimal')" class="mt-2" />
                            </div>

                            <div>
                                <x-input-label for="harga_maximal" :value="__('Harga Maximal')" />
                                <x-text-input id="harga_maximal" class="block mt-1 w-full" type="number" name="harga_maximal" :value="old('harga_maximal')" min="0" step="1000" required />
