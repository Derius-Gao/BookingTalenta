		...
		<div class="max-w-3xl mx-auto sm:px-6 lg:px-8">
			<div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
				<div class="p-6 text-gray-900 dark:text-gray-100">
					...
					<form action="{{ route('pembayarans.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
						@csrf
						...
						<div>
							...
							<input type="number" id="jumlah_harga" name="jumlah_harga" value="{{ old('jumlah_harga') }}" min="0" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700" required />
						</div>
						...
						<div class="grid grid-cols-1 md:grid-cols-2 gap-4">
							...
							<div>
								...
								<select id="status_pembayaran" name="status_pembayaran" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700" required>
									...
									<option value="menunggu verifikasi" {{ old('status_pembayaran') == 'menunggu verifikasi' ? 'selected' : '' }}>Menunggu Verifikasi</option>
									<option value="selesai" {{ old('status_pembayaran') == 'selesai' ? 'selected' : '' }}>Selesai</option>
									<option value="batal" {{ old('status_pembayaran') == 'batal' ? 'selected' : '' }}>Batal</option>
								</select>
							</div>
						</div>
						...
						<div>
							...
							<input type="datetime-local" id="tanggal_pembayaran" name="tanggal_pembayaran" value="{{ old('tanggal_pembayaran') }}" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700" required />
						</div>
