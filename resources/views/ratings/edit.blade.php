	...
	<x-slot name="header">
		<div class="flex justify-between items-center">
			<h2 class="font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight">
				{{ __('Edit Rating') }} #{{ $rating->id_rating }}
			</h2>
					...
					<form action="{{ route('ratings.update', $rating) }}" method="POST" class="space-y-6">
						...
						<div>
							...
							<select id="id_user" name="id_user" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700" required>
								@foreach($users as $user)
									<option value="{{ $user->id }}" {{ old('id_user', $rating->id_user) == $user->id ? 'selected' : '' }}>{{ $user->name }} ({{ $user->email }})</option>
								@endforeach
							</select>
						</div>
						...
						<div>
							...
							<select id="id_talenta" name="id_talenta" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700" required>
								@foreach($talents as $talent)
									<option value="{{ $talent->id_talenta }}" {{ old('id_talenta', $rating->id_talenta) == $talent->id_talenta ? 'selected' : '' }}>{{ $talent->user->name }} - {{ $talent->spesialisasi }}</option>
								@endforeach
							</select>
						</div>
						...
						<div>
							...
							<select id="id_booking" name="id_booking" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700" required>
								@foreach($bookings as $booking)
									<option value="{{ $booking->id_booking }}" {{ old('id_booking', $rating->id_booking) == $booking->id_booking ? 'selected' : '' }}>
						...
						<div class="grid grid-cols-1 md:grid-cols-2 gap-4">
							<div>
								...
								<input type="number" id="score_rating" name="score_rating" value="{{ old('score_rating', $rating->score_rating) }}" min="1" max="5" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700" required />
							</div>
							<div>
								<label for="komentar" class="block text-sm font-medium text-gray-700 dark:text-gray-300">Komentar</label>
								<input type="text" id="komentar" name="komentar" value="{{ old('komentar', $rating->komentar) }}" maxlength="255" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700" required />
							</div>
						</div>
